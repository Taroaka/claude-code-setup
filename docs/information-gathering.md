...
- **canonical synopsis**（誰が/何を求め/何が起き/どう決着するか）
- **beat sheet（時系列の箇条書き10–20個）**
- **登場人物/対立/賭け金/舞台**の最小セット

### 禁止（アンチパターン）

- canonical synopsis が曖昧なまま、裏話・逸話・小ネタ・派生作品の話題を先行させる
- 「英雄譚として適しているか」を語るが、肝心の**ストーリーの骨**が出力されていない

### 英雄譚適性の検証は「骨」が確定してから

英雄の旅/普遍的構造の検証は、上記の beat sheet を根拠に行う（推測だけで結論しない）。

## 必須: 調査情報の scene_id への配賦（Scene mapping）

調査で集めた情報（facts / hooks / tensions / open questions）が「どこで使うべきか」不明だと、
story/script が薄くなる（使い所が無いので捨てられる）か、逆に裏話・小ネタが前に出てストーリーの骨が崩れる。

そのため、調査の段階で **scene_id を保持**し、情報を配賦する。

### ルール（運用）

- scene は原則 **最低20分割（scene_id: 1..20）** の骨格を作る
- **全体に効く情報**（テーマ/対立/結末の意味など）は opening/ending に寄せる（例: 1, 19, 20）
- **途中のネタ**（伏線/逸話/論争点など）は該当 scene_id に割り当てる（複数可）
